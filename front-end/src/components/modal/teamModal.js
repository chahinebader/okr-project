...
query {
getMembre(company:"${localStorage.getItem('company')}"){
  _id,
  name,
  status
} 
...
mutation ToDo($name: String!, $color: String!, $users:[String!]!, $creator: String!){
    createTeam(teamInput:{name:$name,color:$color,users:$users,creator: $creator,company:"${localStorage.getItem('company')}" }){
      _id  
    }
}
`;
...
class ModalComponent extends React.Component {
  constructor(props) {
    ...
    let obj=[];
   for(const item of this.props.data.getMembre){
  ...
  render() {
    ...
    return (
       ...
       <div>       
        <Button pill onClick={this.toggle}> <Icon name='add' /> ADD Team</Button>
        ...
        <Mutation mutation={STAR_Mutation}>
          ...
          <Form onSubmit={e => {
           ...
           postMutation({variables:{name: this.state.name, color: this.state.color, users: obj, creator: localStorage.getItem('userId') }});
      ...
      <FormGroup>
        ...
        <FormInput id="#username" placeholder="Username" onChange={(e)=> this.setState({name:e.target.value})}/>
      </FormGroup>
     ...
     <div className="reactTagContainer">
      ...
      <FormSelect  onChange={(e) => this.setState({color: e.target.value})}>
