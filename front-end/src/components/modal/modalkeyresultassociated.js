...
const GET_OBJECTIF_TREE = gql`query ($id: String!) {
  objectiftreebyid(id:$id) {
  name
  description
  progression
  date_begin
...
return (
   <Query query={GET_OBJECTIF_TREE} pollInterval={5000} variables={{id}}>
    {({ loading, error, data }) => {
    ...
    <Progress theme="success" value={node.progression}><span className="progressValue">{node.progression.toFixed(2)}%</span></Progress>
      ...
      return (
        <div>       
        <Button theme="success" outline onClick={this.toggle}> KR Linked</Button>
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <OrgChart tree={data.objectiftreebyid} NodeComponent={MyNodeComponent}/>
          <ModalFooter>
